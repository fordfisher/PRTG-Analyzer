...
def _parse_tech_breakdown(text: str) -> Dict[str, int]:
    out: Dict[str, int] = {}
    for m in TECH_BREAKDOWN_RE.finditer(text):
...
def _parse_ts(ts_str: str) -> Optional[datetime]:
    """Parse log timestamp (YYYY-MM-DD HH:MM:SS.ffffff); normalizes space to T for fromisoformat."""
    if not ts_str:
        return None
    try:
...
def _normalize_error_text(text: str) -> str:
    """
    Normalize errors so similar ones group together:
    - remove obvious numeric identifiers and hex-ish tokens
    - collapse whitespace
    """
    t = BRACED_GUID_RE.sub("{<guid>}", text)
    t = GUID_RE.sub("<guid>", t)
    t = re.sub(r"\bTId\s+\d+\b", "TId <id>", t)
    t = re.sub(r"\b0x[0-9a-fA-F]+\b", "0x<hex>", t)
...
def parse_core_log(path: Path) -> CoreLogResult:
    ...
    try:
        for raw_line in stream:
            ...
            is_restart_marker = "Logger initialized" in line and RESTART_MARKER_RE.search(line)
            if current_snapshot is None or is_restart_marker:
                _start_new_segment()

            if "Logger initialized" in line and RESTART_MARKER_RE.search(line):
                if ts_str:
            ...
            if level in ("WARN", "ERRR") and ts_str:
                lower = msg.lower()
                if "license" in lower or "activation" in lower or "license check" in lower:
                    continue
