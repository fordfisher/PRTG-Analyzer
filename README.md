...
```
   ...
   This runs PyInstaller with `PRTG_Analyzer.spec`, copies `apply-update.bat` to the folder root, and creates `dist/PyPRTG_CLA_v{version}.zip` for GitHub release. Upload that zip to the release tag (e.g. `v1.5.8`) so the in-app updater can download it.
